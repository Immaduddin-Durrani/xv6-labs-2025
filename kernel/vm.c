...
// walk_for_level: walk page table down to `want_level` and return pointer to PTE there.
// want_level: 0 == leaf (4KB), 1 == 2MB level
static pte_t *
walk_for_level(pagetable_t pagetable, uint64 va, int alloc, int want_level)
{
  for (int level = 2; level > want_level; level--) {
    pte_t *pte = &pagetable[PX(level, va)];
    if (*pte & PTE_V) {
      // DO NOT convert via pa2kva here — the PTE contents in this xv6 hold kernel VA
      pagetable = (pagetable_t)PTE2PA(*pte);
    } else {
      if (!alloc)
        return 0;
      ...
      // store the kernel VA (same style as walk())
...
// demote_superpage: convert a level-1 leaf at va into a normal level-1->level-0 page table.
...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for (a = va; a < va + npages * PGSIZE; a += PGSIZE) {
    // First check if this is part of a superpage
    ...
    if (l1pte && (*l1pte & PTE_V) && ((*l1pte & (PTE_R|PTE_W|PTE_X)) != 0)) {
      ...
      if ((a % SUPERPAGE_SIZE) == 0 && (va + npages * PGSIZE) - a >= SUPERPAGE_SIZE) {
        // We're unmapping a full superpage
        if (do_free) {
          superfree((void*)PTE2PA(*l1pte));
        }
        *l1pte = 0;
        a += SUPERPAGE_SIZE - PGSIZE; // Skip the rest of the superpage
        continue;
      } else {
        // Partial unmap of superpage - demote it first
        if (demote_superpage(pagetable, a) != 0)
          panic("uvmunmap: demote_superpage failed");
        // Now fall through to handle as normal pages
      }
    }
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for (i = 0; i < sz; i += PGSIZE) {
    // First check for superpages
    pte_t *l1pte = walk_for_level(old, i, 0, 1);
    if (l1pte && (*l1pte & PTE_V) && ((*l1pte & (PTE_R|PTE_W|PTE_X)) != 0)) {
      // Found a superpage mapping in parent
      if ((i % SUPERPAGE_SIZE) == 0 && sz - i >= SUPERPAGE_SIZE) {
        ...
        if (child_block) {
          ...
          i += SUPERPAGE_SIZE - PGSIZE; // Skip the rest of superpage
          continue;
        }
        // If superalloc fails, fall through to copy as normal pages
      }
    }
...
mappages_super(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  pte_t *pte = walk_for_level(pagetable, va, 1, 1); // want level 1
  if (pte == 0)
    return -1;
  if (*pte & PTE_V)
    return -1; // already mapped
  *pte = PA2PTE(pa) | perm | PTE_V; // set PA and perms (include PTE_R/PTE_W etc in perm)
  return 0;
}
