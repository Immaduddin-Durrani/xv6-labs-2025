// Enhanced xv6 shell (user/sh.c)
// Features:
//  - interactive-only prompt (no $ when reading from file)
//  - built-in wait
//  - command history (history, !N to rerun)
//  - simple tab completion (first match)
//  - support for quoted arguments ("..." and '...')
...
runcmd(struct cmd *cmd)
{
    ...
    switch(cmd->type){
        ...
        case EXEC:
            ...
            // Built-in commands
            if(strcmp(ecmd->argv[0], "wait") == 0){ wait(0); exit(0); }
            if(strcmp(ecmd->argv[0], "history") == 0){ show_history(); exit(0); }
...
void add_history(char *cmd)
{
    if(history_count < HISTORY_SIZE)
        history[history_count++] = xstrdup(cmd);
    else{
        free(history[0]);
        for(int i=1;i<HISTORY_SIZE;i++) history[i-1] = history[i];
        history[HISTORY_SIZE-1] = xstrdup(cmd);
    }
}
    ...
    while(getcmd(buf, sizeof(buf), interactive) >= 0){
        ...
        if(cmd[0] == '!' && cmd[1] >= '0' && cmd[1] <= '9'){
            ...
            if(idx >= 0 && idx < history_count)
                ...
                printf("No such history command\n");
                continue;
            }
            cmd = buf;
        }
